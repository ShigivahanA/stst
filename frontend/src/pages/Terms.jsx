...
import { Scale, Shield, AlertTriangle, FileText, ChevronRight, Mail, MapPin, Globe, CheckSquare, Loader2, ArrowLeft } from 'lucide-react'
...
export default function Terms() {
  ...
  const renderBody = (bodyText) => {
    ...
    return paragraphs.map((para, pIdx) => {
      ...
      // Check if it looks like a Definition/Key-Value (e.g. "Company Name: STAT...")
      ...
      if (/^\d+\s*\.\s*/.test(para.trim())) {
        return (
          <div key={pIdx} className="flex items-start gap-3 my-2">
            ...
            <span className="text-sm sm:text-base pt-0.5">{para.replace(/^\d+\s*\.\s*/, '')}</span>
          </div>
        )
      }
